...
- **Komenda restartu po LAN nie jest jeszcze ustalona** (`REBOOT_DP = None`) — dopóki tak jest, mechanizm restartu śpi i zachowanie skryptu jest identyczne jak przed jego dodaniem. Protokół lokalny TinyTuya nie zna komendy „reboot", więc musi to być punkt danych **tylko do zapisu** (takim jest u nas np. DP 140 — działa, choć nie widać go w żadnym odczycie). Sprawdzone i odrzucone na żywym urządzeniu: **DP 142** (tuya-local opisuje go jako `class: restart`) nie reaguje na firmware 2.9.4 w żadnym z trzech formatów wartości — 126 próbek, zero spadków napięcia Control Pilot; **DP 188** działa, ale tylko odświeża dane. **DP 141** (tuya-local: „Reset") pozostaje niesprawdzony — uwaga, jeśli okaże się resetem do ustawień fabrycznych, wallbox traci sparowanie i local key, więc nie ruszać bez działającego dostępu do Tuya IoT API, którym można ten klucz odzyskać
