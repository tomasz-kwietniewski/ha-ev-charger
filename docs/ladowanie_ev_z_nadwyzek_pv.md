...
Logi pokazały coś innego i to była pierwsza lekcja: **pamięć nie jest dobrym źródłem danych diagnostycznych**. Tamtego dnia od 10:25 do 15:34 nadwyżka sięgała 5-8 kW, produkcja dochodziła do 8,4 kW, a magazyn stał na 100%. Warunki idealne. Ładowanie nie ruszyło ani razu.

Objawy, które zapamiętałem, były prawdziwe, tylko dotyczyły **dwóch minut**, nie całego popołudnia. O 15:35 sesja w końcu ruszyła, ale słońce zdążyło schować się za chmury: produkcja spadła do 700 W, a ładowarka pobierała 3,8 kW. Różnica szła z magazynu. Skrypt policzył to poprawnie, w ciągu trzydziestu sekund wysłał STOP - i wallbox wykonał go dopiero po dwóch minutach. Stąd rozładowany magazyn: około 0,13 kWh.

Ale prawdziwe znalezisko było gdzie indziej. **Samochód przez te pięć godzin w ogóle nie był podpięty.** A skrypt przez cały ten czas:

- wysłał **72 komendy START** do pustego gniazda,
- wygenerował **70 ostrzeżeń** w logu,
- i rzucił **dwa fałszywe alarmy o awarii**, z których pierwszy wisiał **pięć godzin**.

Przyczyna okazała się wstydliwie prosta. Ładowarka ma stany `IDLE` i `SLEEP`, które znaczą „nie widzę żadnego auta". W moim kodzie oba siedziały na liście stanów gotowości do ładowania. Skrypt wchodził więc w tryb solarny, wysyłał START, a watchdog widział „chcemy ładować, a pomiar stoi w miejscu" i uznawał to za zawieszenie. Pomiar stał, bo prąd nie płynął. Nie było czym.

**I to jest gorsze niż zmarnowana energia.** Cała wartość powiadomienia opiera się na tym, że kiedy przychodzi, to coś znaczy. Alarm, który myli się przy każdym odpiętym aucie w słoneczny dzień, po tygodniu przestaje się czytać - a wtedy przepada ten jeden prawdziwy, dla którego cały mechanizm powstał. Zbudowałem sobie wilka, który woła „owce".

Naprawa wzięła się z rzeczy, którą tydzień wcześniej zapisałem jako ciekawostkę bez zastosowania: **napięcia Control Pilot**. Ładowarka przez cały czas mówi, czy kabel siedzi w aucie. Około 12 V znaczy „puste gniazdo", 9 V „podłączone", 6 V „ładuję" - stany A, B i C z normy IEC 61851. Wystarczyło zacząć to czytać i pytać urządzenie o fakt, zamiast wnioskować ze statusu, który znaczy co innego, niż zakładałem.
...
Jedna rzecz świadomie została niedokończona. Chciałem dorzucić zabezpieczenie na wypadek, gdyby zawieszenie zamroziło samo napięcie Control Pilot: wtedy ładowarka w nieskończoność twierdziłaby „nie ma auta", a skrypt by jej wierzył. Zaplanowałem alarm po godzinie takiego stanu - i **odrzuciłem ten pomysł po sprawdzeniu danych**. Przy odpiętym aucie pomiar realnie stoi godzinami; tamtego dnia ten sam odczyt trwał od 10:30 do 15:32. Takie zabezpieczenie produkowałoby dokładnie te fałszywe alarmy, które właśnie usuwałem. Lepszy jawnie opisany brak pokrycia niż mechanizm, który udaje ochronę, a w praktyce hałasuje.

**Wniosek:** warto oddzielać „urządzenie nie robi tego, co chcę" od „urządzenia nie ma z kim pracować". To brzmi banalnie, dopóki nie okaże się, że przez trzy miesiące traktowało się jedno jak drugie - i że diagnostyka, która miała pilnować systemu, sama stała się źródłem fałszywych sygnałów.
